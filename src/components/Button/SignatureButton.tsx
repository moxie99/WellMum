...
import { Box, Icon, Image } from "@/components/Base";
...
const SignatureButton = ({
  ...
  const _onSaveEvent = (result: any) => {
    if (!result) {
      return showToast("Sign on the app to save");
    }
    ...
    try {
      await launchCamera(
        ...
        (res) => {
          ...
          // @ts-ignore
          onChangeText({
            encoded: res?.assets[0]?.base64,
            pathName: res?.assets[0]?.fileName,
          });
        },
      );
    } catch {}
  ...
  const handleLaunchImageLibrary = async () => {
    try {
      await launchImageLibrary(
        ...
        (res) => {
          if (res.didCancel) {
            return showToast("User cancelled image picker");
          }
          if (res.errorCode) {
            return showToast(res.errorCode);
          }
